...
unsigned char *HevcAddHdrInfoToSps(ST_HEVC_SPS *pstSps, unsigned int * puiStreamLen)
{
    ...
    if (pstSps->num_short_term_ref_pic_sets){
        for (int j = 0; j < pstSps->num_short_term_ref_pic_sets; j++){
            //if (i != 0)
            //{
            //HevcWriteWord(&stBs, 1, pstSps->short_term_ref_pic_set[0].inter_ref_pic_set_prediction_flag);
            //}

            //if (!pstSps->short_term_ref_pic_set[0].inter_ref_pic_set_prediction_flag)
            //{
            HevcWriteUE(&stBs,  pstSps->short_term_ref_pic_set[0].num_negative_pics);
            HevcWriteUE(&stBs,  pstSps->short_term_ref_pic_set[0].num_positive_pics);
            for (int i = 0; i < pstSps->short_term_ref_pic_set[0].num_negative_pics; i++){
                HevcWriteUE(&stBs,  pstSps->short_term_ref_pic_set[0].delta_poc_s0_minus1[i]);
            }
            ...
            for (int i = 0; i < pstSps->short_term_ref_pic_set[0].num_positive_pics; i++){
                HevcWriteUE(&stBs,  pstSps->short_term_ref_pic_set[0].delta_poc_s1_minus1[i]);
            }
    ...
    if (pstSps->vui_parameters_present_flag){
        ...
        //这边有个逻辑，就是video_signal_type_present_flag如果为1 才会去读取colour_description_present_flag，反言之，则不读
